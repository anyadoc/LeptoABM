...
[
  ...
  week                           ;;counter for weeks; resets to 1 after 52  ;ABelsare_edits: replaces year in the previous version
  ;;owned dogs
...
if r-init != 0 [
  ...
  if (Output_file? = "YES") [
    if (file-exists? (word OutputFileName) = FALSE)[
      ...
      file-print "Year,AnnualDLinc/10K,AnnualRLinc/10K"
      file-close]]
reset-ticks
end
;--------- END SETUP --------
...
to d-get-infected
  ;;ABelsare_edit: as per the code, if both serovars are present on a patch, a dog will be infected with dog serovar.17Aug17 changed code so that equal chances of getting infected with either serovar.
  let serovar-list []
...
to prob-infection-dogserovar
  if (d-SIR-dog-S = TRUE and d-SIR-rod-S = TRUE)[   ;;17Aug17 ABelsare_edits: added and d-SIR-rod-S = TRUE to prevent coinfection
    ;if (any? odogs-here with [d-SIR-dog-Shed = TRUE]) or (any? sdogs-here with [d-SIR-dog-Shed = TRUE])[ ;;ABelsare_edits: POTENTIAL ERROR HERE: SHOULD BE any? odogs-here
      ...
      if (random-float 1 < 0.01)[
        ...
        set d-rate-decay-dog (0.15 + random-float 0.2)
        set d-SIR-dog-S FALSE
        set d-death-prob (d-death-prob * 1)       ;;18Aug17 ABelsare_edits: changed from 1.1 to 1 as no change in baseline death rate if dog is infected with seroD;; also changed set d-death-prob (0.0429 * 1) to set d-death-prob (d-death-prob * 1)
        ]
      ;]
  ]
end
to prob-infection-pdogserovar
  if (d-SIR-dog-S = TRUE and d-SIR-rod-S = TRUE)[               ;;17Aug17 ABelsare_edits: only susceptible dogs get infected from the patch
    if (random-float 1 < p-inf-dog)[
      ...
      set d-rate-decay-dog (0.15 + random-float 0.2)
      set d-SIR-dog-S FALSE
      set d-death-prob (d-death-prob * 1)                    ;;18Aug17 ABelsare_edits: changed from 1.1 to 1 as no change in baseline death rate if dog is infected with seroD;; also ;;18Aug17 ABelsare_edits:set d-death-prob d-death-prob * 1
      ]
    ]
end
to prob-infection-rodentserovar
  if (d-SIR-rod-S = TRUE and d-SIR-dog-S = TRUE) [     ;;17Aug17 ABelsare_edits: added ..and d-SIR-dog-S = TRUE ...to prevent coinfection
...
to prob-infection-prodentserovar
  if (d-SIR-rod-S = TRUE and d-SIR-dog-S = TRUE) [
    ;let prob-inf (random-float 1)
...
to r-get-infected
  ;;
  ;;ABelsare_edit: as per the code, if both serovars are present on a patch, a rodent will be infected with dog serovar.17Aug17 changed code so that equal chances of getting infected with either serovar.
  let rserovar-list []
...
839
OutputFileName
c:/Users/PC/Desktop/HumanLeptoIncidence.csv
1
0
String
